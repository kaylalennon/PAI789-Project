...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 30 10:06:08 2022
...
sm = plt.cm.ScalarMappable(cmap=my_colormap, 
        norm=plt.Normalize(vmin=min(geodata["Female College Ratio"]),vmax=max(geodata["Female College Ratio"])))
...
ax2.set_xticklabels([])
ax2.set_yticklabels([])
ax2.set_title("Female College Ratio (Pop 15+ in College : Pop 15+ Not In College")
...
for index,state in usa_more.iterrows():
    ...
    if state['NAME'] in ("Alaska", "Hawaii"):
        st_name = state['NAME']
        facecolor = my_colormap( state["Female College Ratio"] / max(geodata["Female College Ratio"] ))
...
fig.savefig("FeCollegeRatio.png", dpi=300)