...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 30 09:58:47 2022
...
sm = plt.cm.ScalarMappable(cmap=my_colormap, 
        norm=plt.Normalize(vmin=min(geodata["Female Enrollment Rate"]),vmax=max(geodata["Female Enrollment Rate"])))
...
ax2.set_xticklabels([])
ax2.set_yticklabels([])
ax2.set_title("Female College Enrollment Rate Age 15+")
...
for index,state in usa_more.iterrows():
    ...
    if state['NAME'] in ("Alaska", "Hawaii"):
        st_name = state['NAME']
        facecolor = my_colormap( state["Female Enrollment Rate"] / max(geodata["Female Enrollment Rate"] ))
...
fig.savefig("FeEnrollRate.png", dpi=300)